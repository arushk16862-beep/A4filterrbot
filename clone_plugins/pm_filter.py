...
async def cb_handler(client: Client, query: CallbackQuery):
    # custom clone pages
    if query.data in ("help", "abt", "about"):
        cs = await get_clone_settings(client)
        if query.data == "help":
            text = render(cs.get("help_message") or "Help", query.message)
            kb = markup(cs.get("help_buttons"))
        else:
            text = render(cs.get("about_message") or "About", query.message)
            kb = markup(cs.get("about_buttons"))
        if kb is None:
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('ʙᴀᴄᴋ', callback_data='help'),
            InlineKeyboardButton('7ᴜʟ', callback_data='rfrsh')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("ᴄʀᴇᴀᴛᴏʀ", url="t.me/Lallu_Tg")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id
