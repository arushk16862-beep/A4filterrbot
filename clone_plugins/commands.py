...
from clone_plugins.clone_settings import get_clone_settings, markup, render
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        cs = await get_clone_settings(client)
        # Always use clone-specific text/buttons; this keeps every cloned bot independent.
        text = render(cs.get("start_message") or script.CLONESTART_TXT, message)
        kb = markup(cs.get("start_buttons"))
        if kb is None:
            kb = InlineKeyboardMarkup([[
                InlineKeyboardButton('🕵️ Help', callback_data='clone_custom_help'),
                InlineKeyboardButton('🤷 About', callback_data='clone_custom_about')
            ],[
                InlineKeyboardButton('➕ Add Me To Group', url=f'http://t.me/{clonedme.U_NAME}?startgroup=true')
            ]])
        await message.reply_text(text=text, reply_markup=kb, parse_mode=enums.ParseMode.HTML)
        return
    ...
    if not files_:
        ...
        try:
            cs = await get_clone_settings(client)
            if cs.get('verify_bot_enabled') and not await check_verification(client, message.from_user.id):
                btn = [[
                ...
                await message.reply_text(
                    text=f"<b>{(cs.get('verify_message') or 'You are not verified! Kindly verify to continue.').replace(chr(60), '&lt;').replace(chr(62), '&gt;')}</b>",
                    protect_content=True,
                    reply_markup=InlineKeyboardMarkup(btn)
                )
                return
    ...
    if cs.get('verify_bot_enabled') and not await check_verification(client, message.from_user.id):
        ...
        return
    cs = await get_clone_settings(client)
    custom_file_message = cs.get("file_send_message") or ""
    if custom_file_message:
        try:
            await client.send_message(message.from_user.id, render(custom_file_message, message))
        except Exception:
            pass
    sent = await client.send_cached_media(
        ...
        caption=f_caption,
        protect_content=True if pre == 'filep' else False,
        reply_markup=markup(cs.get("file_buttons"))
    )
    if cs.get("auto_delete_files"):
        asyncio.create_task(_delete_later(sent, cs.get("auto_delete_seconds", 600)))
